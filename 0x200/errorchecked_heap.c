...
int main(int argc, char *argv[]) {
  ...
  char_ptr = (char *)errorchecked_malloc(mem_size); // Allocting heap memory
  ...
  *int_ptr = 31337; // Put the value of 31337 where int_ptr is pointing.
  ...
  char_ptr = (char *)errorchecked_malloc(15); // Allocting heap memory
...
void *errorchecked_malloc(unsigned int size) { // An error-checked malloc() function
  void *ptr;
  ptr = malloc(size);
  if(int_ptr == NULL) { // Error checking, in case mallloc fails
